...
| `[E103] ... config.image.tag is required ... when config.image.source is "registry"` | A module instance sets `config.image.source: registry` without also setting `config.image.tag`. NOTE: the renderer does not yet consume `config.image.source`/`tag`, so this is enforced ahead of that wiring. | Set `config.image.tag` to a version, or switch back to `config.image.source: build`. |
