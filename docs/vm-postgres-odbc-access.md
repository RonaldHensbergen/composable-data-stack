...
The `postgres` module binds the database port to `127.0.0.1` on the host
(`modules/warehouse/postgres/module.yaml`) and `cds security` rule
`CDS-SEC-021` (severity: high) flags any database service with a published
host port (`portExposure: host-published`; any colon-carrying port
mapping). The profile config never declares a published host port, so the
rule never triggers. Rather than opening the database directly to the
...
![Postgres ODBC connection through an SSH tunnel](https://raw.githubusercontent.com/RonaldHensbergen/composable-data-stack/main/docs/diagrams/architecture/vm_postgres_odbc_tunnel.svg)
...
- `cli/resources/rule-set.json` — `CDS-SEC-021` (database published
  externally)
- `docs/threat-model.md` — T8 network exposure threat
