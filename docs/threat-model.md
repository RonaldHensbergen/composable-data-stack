...
![CDS trust boundaries](https://raw.githubusercontent.com/RonaldHensbergen/composable-data-stack/main/docs/diagrams/architecture/trust_boundaries.svg)
...
| # | Attack path | Boundary | Current control | Residual risk / owning issue |
| --- | --- | --- | --- | --- |
| T19 | Image pulled by `:latest` or unpinned tag, silently changes underneath a deployment | C | Not defined as rule-engine rules; enforcement comes entirely from `cli/image_verification.py`'s digest-pin checks (reported as CDS-SEC-050/051 findings) | Covered by `cli/image_verification.py`, not by rule-engine rules. |
| T20 | Image pulled without a digest pin, tag can be repointed at the registry | C | Not defined as a rule-engine rule; enforcement is `cli/image_verification.py`'s digest-pin check (reported as a CDS-SEC-051 finding) | Covered by `cli/image_verification.py`, not by a rule-engine rule. |
| T21 | Image pulled from an untrusted/typo-squatted registry | C | Not defined as a rule-engine rule; `cli/image_verification.py` enforces a case-insensitive registry allowlist (PR #337) | Covered. |
