...
Generated automatically by `scripts/render_support_notice.py` (see
[`docs/security-support-policy.md`](docs/security-support-policy.md)) and
appended by `.github/workflows/release.yml`. Do not remove it or hand-write
a substitute; `scripts/check_release_notes_support.py` fails the release
...
- [ ] [`docs/cra-scope-decision.md`](docs/cra-scope-decision.md) reassessed
  ...
  release
- [ ] Release notes include the `## Support` section generated by
  `scripts/render_support_notice.py` (verified automatically by
...
For a Critical/High-severity vulnerability, follow the severity-based
delivery targets and `hotfix/*` branch flow defined in
[`docs/security-support-policy.md`](docs/security-support-policy.md#3-security-update-delivery)
instead of waiting for the weekly release train. The steps are the same as
an ordinary release (this document) plus:

1. Branch from the affected tag as `hotfix/vX.Y.Z+1`.
2. Apply the minimal fix and a regression test.
3. Tag and push immediately rather than waiting for the Monday
   `version-bump` automation.
4. Reference the GitHub Security Advisory (if one was filed) from the
   `CHANGELOG.md` entry and release notes.
